  ...
  const text = $derived(
    en
      ? {
          ...
          usbOn:
            "Enabled. USB selective suspend can add input latency on mice and keyboards.",
          unknown: "Could not read this setting.",
          confirmTitle: "Disable power setting",
          confirmMsg:
            "This applies immediately, no restart needed. The previous value is saved and can be restored here.",
          restorableHint:
            "Applied before via PaceDock; you can restore the original value here.",
          abHint:
            "Tip: measure a real game before and after (Measurement tab) to verify the effect.",
          confirm: "Confirm",
          cancel: "Cancel",
        }
      : {
          ...
          confirmTitle: "停用電源設定",
          confirmMsg:
            "立即生效,無需重新啟動。套用前的原值會被記錄,可在此還原。",
          restorableHint: "此前已由 PaceDock 套用,可在此還原原值。",
          abHint:
            "提示:可於「量測」分頁在套用前後各測一次實際遊戲量測,對照驗證效果。",
          confirm: "確認",
          cancel: "取消",
        },
  );
...
{#if status}
  <section
    class="panel card"
    class:ok={status.pcieAspm === 0}
    class:warn={status.pcieAspm != null && status.pcieAspm !== 0}
  >
    <h2>{text.aspmTitle}</h2>
    <p class="card-status">
      {status.pcieAspm == null
        ? text.unknown
        : status.pcieAspm === 0
          ? text.off
          : text.on}
  ...
  <section
    class="panel card"
    class:ok={status.usbAc === 0 && status.usbDc === 0}
    class:warn={(status.usbAc != null && status.usbAc !== 0) ||
      (status.usbDc != null && status.usbDc !== 0)}
  >
    <h2>{text.usbTitle}</h2>
    <p class="card-status">
