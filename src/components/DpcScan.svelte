<script lang="ts">
  ...
  const text = $derived(
    en
      ? {
          title: "DPC noise scan",
          measure: "Scan for 3 seconds",
          measuring: "Scanning…",
...
<section class="dpc-panel" aria-busy={scanning}>
  <div class="heading">
    ...
    <button
      ...
      disabled={scanning}
      aria-busy={scanning}
      onclick={scan_now}>{scanning ? text.measuring : text.measure}</button
    >
  </div>
  ...
  <div role="status" aria-live="polite">
    ...
    {:else if scan}
      <p>
        {text.at}: {new Date(scan.sampledAt).toLocaleTimeString(
          en ? "en-US" : "zh-TW",
        )} ({scan.sampleSecs}s)
      </p>
      ...
      {#if scan.offenders.length}
        <table>
          <thead
            ><tr
              ><th>{text.driver}</th><th>{text.count}</th><th>{text.total}</th
              ><th>{text.max}</th></tr
            ></thead
          >
          <tbody>
            {#each scan.offenders as o}
              <tr
                ><td>{o.driver}</td><td>{o.count.toLocaleString()}</td><td
                  >{o.totalDurationMs.toFixed(2)}</td
                ><td>{o.maxDurationMs.toFixed(2)}</td></tr
              >
            {/each}
          </tbody>
        </table>
