...
export const getTimerGlobalEnabled = () =>
  invoke<boolean | null>("get_timer_global_enabled");
...
export const listTimerExempts = () =>
  invoke<TimerExemptEntry[]>("list_timer_exempts");
...
export const performPortableUpdate = () =>
  invoke<void>("perform_portable_update");

// GPU 基準測試相關
export const enumerateGpus = () => invoke<GpuDevice[]>("enumerate_gpus");
export const getBenchmarkState = () =>
  invoke<BenchmarkState>("get_benchmark_state");
export const listBenchmarkSessions = () =>
  invoke<SessionSummary[]>("list_benchmark_sessions");
...
export const getBenchmarkStorageInfo = () =>
  invoke<StorageInfo>("get_benchmark_storage_info");
export const getGpuAffinityPolicy = (instanceId: string) =>
  invoke<AffinityPolicy>("get_gpu_affinity_policy", { instanceId });
export const restorePreviousGpuAffinity = () =>
  invoke<void>("restore_previous_gpu_affinity");
...
export const getCoreCandidates = () =>
  invoke<import("./types").CoreTarget[]>("get_core_candidates");
export const getQuickSchedule = (config: BenchmarkConfig) =>
  invoke<import("./types").QuickSchedule>("get_quick_schedule", { config });
export const applyGpuCore = (
  instanceId: string,
  coreId: number,
  sessionId: string | null,
) => invoke<void>("apply_gpu_core", { instanceId, coreId, sessionId });
...
export const startGameCapture = (
  pid: number,
  title: string,
  durationSecs: number,
  gpuInstanceId: string | null,
) =>
  invoke<GameCaptureRecord>("start_game_capture", {
    pid,
    title,
    durationSecs,
    gpuInstanceId,
...
export const listGameCaptures = () =>
  invoke<GameCaptureRecord[]>("list_game_captures");
export const deleteGameCapture = (id: string) =>
  invoke<void>("delete_game_capture", { id });
...
export const applyMsi = (instanceId: string) =>
  invoke<void>("apply_msi", { instanceId });
export const restoreMsi = (instanceId: string) =>
  invoke<void>("restore_msi", { instanceId });

// 電源微調(USB 選擇性暫停 / PCIe ASPM)
export const getPowerTweaks = () =>
  invoke<PowerTweaksStatus>("get_power_tweaks");
...
// 中斷落點驗證與 DPC 掃描
export const verifyInterruptAffinity = (
  instanceId: string,
  expectedLps: number[],
) =>
  invoke<InterruptVerification>("verify_interrupt_affinity", {
    instanceId,
    expectedLps,
  });
